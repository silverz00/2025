...
def photo(query: str, seed: int = 0, w: int = 900, h: int = 600) -> str:
    """Return a (semi-)stable Unsplash image URL for a keyword."""
...
month = st.sidebar.select_slider("여행 예정 월(선택)", options=list(range(1,13)), value=8)
...
for d in DSTS:
    ...
    if region_pref:
        if d.region in region_pref:
            ...
            score -= 0.2
    # a small random tie-breaker that is stable per destination
    rnd = (hash(d.name) % 100) / 1000.0
    score += rnd
...
for d in results:
    ...
    with st.expander("📸 사진 더 보기 / 🍽️ 대표 음식 보기", expanded=True):
        # Gallery
        st.markdown("#### 📸 갤러리")
...
st.markdown(
    """
    **Tip**: 사이드바에서 예산/지역/월을 바꾸면 추천이 즉시 업데이트됩니다.  
